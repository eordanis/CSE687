...
#pragma once
#include "Test.h"
#include "../Header/IMap.h"

typedef IMap* (*CreateObjectofMap)();

TEST(MapTest, TestMap)
{
	std::string mapDllPath("./MapDLL.dll");
	...
	if (dll_handle) {
		...
		if (pCreateObjectofMapPtr) {
			...
			map->setInputFileName("testMapFile1.txt");
			map->setTempFileName(tmpFileName2);
			map->map("testMapFile1.txt", fileContentsWrite);
			delete map;
